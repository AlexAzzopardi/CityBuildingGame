...
public class Collect_Iron_1 : MonoBehaviour {
    ...
    void FixedUpdate()
    {
        ...
        if (harvested == true && Time.time > next_time)
        {
            ...
            if (data_manager_script.Check_Resources(4) + 60 <= data_manager_script.Get_Max_Storage())
            {
                //Add resources to inventory
                data_manager_script.Change_Resources(4, 60);
            }

            //Checks to see if it will go over the maximum storage.  
            if (data_manager_script.Check_Resources(2) + 40 <= data_manager_script.Get_Max_Storage())
            {
                //Add resources to inventory
                data_manager_script.Change_Resources(2, 40);
            }
